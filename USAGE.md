...
|---|---|
| `brec enrich` | приписывает файлам трассы пакет-владельца по `rpm-dump.txt`, кладёт результат рядом с трассой |
...
### Атрибуция пакетов лежит рядом с трассой, а не в ней

`brec enrich` сопоставляет каждый файловый узел трассы с пакетом-владельцем по
`rpm-dump.txt` и пишет результат отдельным файлом:

```bash
# в контейнере, где шла сборка
rpm -qa --qf '[%{FILENAMES}\t%{NAME}\t%{NEVRA}\n]' > rpm-dump.txt

python3 -m brec enrich pkg-build.out rpm-dump.txt
# → pkg-build.provenance.json, сама pkg-build.out не изменена
```

Почему отдельным файлом. Трасса это доказательство: то, что записал
трассировщик, наблюдая сборку. Атрибуция пакетов выводится позже, по чужой
базе, возможно на другой машине и через месяцы после сборки. Дописанная внутрь
`.out`, она становится неотличима от наблюдения, а хеш трассы перестаёт
сходиться с тем, что был сразу после сборки. Поэтому вывод идёт в sidecar, а
трасса остаётся байт в байт.

Остальные подкоманды подхватывают `<трасса>.provenance.json` рядом сами; путь
можно задать явно флагом `--provenance FILE`. Sidecar, собранный по другой
трассе, не применяется молча: каждая запись сверяется с узлом по пути, о
расхождении команда предупреждает и такие записи пропускает.

Трассы, обогащённые старой версией (тройки `b:rpm_name` дописаны внутрь `.out`),
продолжают читаться. Если рядом есть ещё и sidecar, верх берёт он: это более
позднее и явное утверждение.
...
ложный RED. Альтернатива: заранее прогнать `python3 -m brec enrich <out> <rpm-dump>`,
тогда вердикт подхватит sidecar рядом с трассой сам.
...
| `--rpm-dump` | `rpm -qa --qf '[%{FILENAMES}\t%{NAME}\t%{NEVRA}\n]'` | атрибуция файлов, если для трассы нет sidecar от `brec enrich`, и резолв файловых возможностей (`/bin/sh`) |
...
└── brec/               # анализ трассы: пакет и CLI `python3 -m brec`
    ...
    └── provenance/     # бэкенды пакетных баз (rpm) и sidecar-атрибуция
```
